...
# Get Secure Score
    ...
    $tags = Search-AzGraph -Query 'resources | where type == "microsoft.hybridcompute/machines" or type == "microsoft.compute/virtualmachines" | extend Department = tags.Department | project id, name, location, resourceGroup, Department'

    foreach($tag in $tags){


         $json += @"
{ "SubscriptionId":"$SubscriptionId", "Subscription":"$SubscriptionName", "ResourceID":"$($tag.id)", "Name":"$($tag.name)", "Location":"$($tag.location)","resourceGroup":"$($tag.resourceGroup)","Department":"$($tag.Department)"  },
"@
